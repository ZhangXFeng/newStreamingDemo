...
#Java程序日志所在的目录
APP_LOG=/root/kafkaProducer/log


#Java程序主体所在的目录,即classes的上一级目录
APP_HOME=/root/kafkaProducer/

#Java主程序,即main(String[] args)方法类
APP_MAIN=io.transwarp.kafka.consumer.LogConsumer

#classpath参数,包括指定lib目录下的所有jar
CLASSPATH=$APP_HOME/conf:$APP_HOME/stream.jar:/usr/lib/hbase/lib/
...
startup(){
	...
	nohup     $JAVA_HOME/bin/java  -classpath $CLASSPATH $APP_MAIN  &
        getTradeProtalPID
        if [ $tradePortalPID -ne 0 ]; then
